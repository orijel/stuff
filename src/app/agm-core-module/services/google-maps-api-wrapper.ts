...
export class GoogleMapsAPIWrapper {
  ...
  createMap(el: HTMLElement, mapOptions: mapTypes.MapOptions): Promise<void> {
    return this._loader.load().then(() => {
      ...
      overlay.draw = function () {
        this.getPanes().markerLayer.classList.add('markerLayer');
      };
      overlay.setMap(map);
  ...
  createMarker(options: mapTypes.MarkerOptions = <mapTypes.MarkerOptions>{}, addToMap: boolean = true):
    ...
    return this._map.then((map: mapTypes.GoogleMap) => {
      options.animation = (typeof options.animation === 'string') ? google.maps.Animation[options.animation] : options.animation
      if (addToMap) {
        options.map = map;
      }
